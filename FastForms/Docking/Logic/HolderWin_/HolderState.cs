...
public sealed class HolderState
{
	...
	private void MakeSysWin()
	{
		...
		// Create the window
		// =================
		sys = new SysWin(D, clientR => HolderLayout.AdjustClientR(clientR, Docker.TreeType.V is Enums.TreeType.ToolSingle));
		...
		HitTester.ForPassthrough(
			...
			() => Docker.TreeType.V is Enums.TreeType.ToolSingle
		);
		...
		Sys.Evt.WhenPaint.Subs((ref PaintPacket e) =>
		{
			...
			HolderWinPainter.Paint(
				...
				TreeType.V is Enums.TreeType.ToolSingle,
				btns,
		...
		// =============================================
		PaneMover.Setup(Sys, this);


		// Allow user the tear the Holder off (if TreeType != ToolSingle)
		// ==================================
		HolderJerker.Setup(holderNode, btns.IsOverAnyButton);
	}
}
